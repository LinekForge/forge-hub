...
| `formatHints?` | `string` | **可选**。告诉 LLM 该通道支持什么消息格式（如"纯文本，不支持 Markdown"或"支持 MarkdownV2"）。Hub 在入站消息转发时自动注入到消息 meta 的 `format_hints` 字段，LLM 收到消息时即知道回复的格式约束。内置通道全部声明了此字段。 |
...
| `isAllowed(senderId)` | 检查 sender 是否在本通道 allowlist 中。用于入站 allowlist 判定。 |
| `getNickname(senderId)` | 从 allowlist 查找 sender 的昵称，未找到则返回 senderId 本身。 |
| `recordSecurityEvent(params)` | 登记聚合安全事件。Hub 层自动限频（每通道每 sender 最多 1 条/小时）。`params: { sourceUserId, contentType, evidenceId }`。 |
...
interface InboundMessage {
  ...
  targetInstanceId?: string;     // Dashboard 显式指派给某实例时填写
}
```
...
参考 [`telegram.ts`](../hub-server/channels/telegram.ts) 里 `if (!isAllowed(chatId))` 分支——内置通道已全部升级为 **evidence + security event + 静默丢弃** 模式：

```ts
import { recordUnauthorizedEvidence } from "../evidence.js";

if (!isAllowed(senderId)) {
  // 1. 把原始消息写入 evidence vault（磁盘落盘，fh hub security evidence 可查）
  const evidence = recordUnauthorizedEvidence({
    channel: plugin.name,
    ingestMode: "polling",         // 或 "getUpdates" / "chatdb" 等，按你通道的实际方式
    updateId: String(updateId),
    chatId: senderId,
    messageId: msgId ? String(msgId) : null,
    sourceUserId: userId ? String(userId) : null,
    contentType: "text",           // "text" | "photo" | "voice" | "file" 等
    contentMeta: {},               // 可选：文件大小、mime 等非敏感 metadata
  ...
  hub.recordSecurityEvent({
    sourceUserId: userId ? String(userId) : senderId,
    contentType: "text",
...
- 告警文案不要回显原文（`formatUnauthorizedNotice` 会丢弃 rawContent，只保留净化后的 sender 标识）。
