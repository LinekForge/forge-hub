...
2. **`content` 走 `hub.formatUnauthorizedNotice(displayName, senderId, rawContent)`**。它不会回显 `rawContent`；`displayName` / `senderId` 会按外部输入净化、单行化、截断后只用于定位。不要自己拼字符串。**签名只有 3 个参数**——channel name 由 HubAPI factory 从 closure 注入。
