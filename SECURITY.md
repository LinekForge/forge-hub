...
- **Channel 协议是 prompt injection 面**：所有已授权入站消息都会进入 LLM 上下文。内置通道遇到未授权消息时记录 evidence / security event，并只发送固定模板告警，不回显外部原文；告警触发时会同步落盘，用户可立即用 `fh hub security` 查看。`formatUnauthorizedNotice` 仅保留给第三方 plugin / 旧示例自防护；定制 channel plugin 时**务必**验证 sender。
